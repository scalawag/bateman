...
import org.scalawag.bateman.json.{Decoder, JAnyEncoder, JNumber, JNumberDecoder, JObjectCodec}
import test.json.BatemanTestBase

object AutoTest {
  sealed trait BothTrait
  final case class BothClass(a: Int, b: String) extends BothTrait
...
class AutoTest extends BatemanTestBase {
  ...
  it("should generate a case class codec when both encoder and decoder can be generated") {
    import AutoTest._
    import org.scalawag.bateman.json.generic.auto._
    val codec = implicitly[JObjectCodec[BothClass]]
    BothClass(1, "hello") shouldEncodeTo json"""{"a":1,"b":"hello"}"""
    inside(json"""{"a":1,"b":"hello"}""".asRootFocus.decode[BothClass].shouldSucceed) {
      case BothClass(a, b) =>
        a shouldBe 1
        b shouldBe "hello"
    }
  ...
  it("should generate a trait codec when both encoder and decoder can be generated") {
    import AutoTest._
    import org.scalawag.bateman.json.generic.auto._
    val codec = implicitly[JObjectCodec[BothTrait]]
    (BothClass(1, "hello"): BothTrait) shouldEncodeTo json"""{"type":"BothClass","a":1,"b":"hello"}"""
    inside(json"""{"type":"BothClass","a":1,"b":"hello"}""".asRootFocus.decode[BothTrait].shouldSucceed) {
      case BothClass(a, b) =>
        a shouldBe 1
        b shouldBe "hello"
    }
