...
final case class CharStream(chars: LazyList[Char], location: JLocation) {
  @tailrec
    ...
    if (n < 1)
      ...
      (chars.head match {
        ...
        case '\r' if chars.tail.headOption.contains('\n') =>
          CharStream(chars.tail.tail, location.copy(column = location.column + 1))
        case '\r' | '\n' =>
          CharStream(chars.tail, location.copy(line = location.line + 1, column = 1))
        case _ => CharStream(chars.tail, location.copy(column = location.column + 1))
      }).drop(n - 1)
    }
  }
