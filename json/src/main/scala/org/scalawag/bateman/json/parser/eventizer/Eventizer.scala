...
package org.scalawag.bateman.json.parser.eventizer

import cats.syntax.either._
import org.scalawag.bateman.json.JErrorFormatters
import org.scalawag.bateman.json.parser.{SyntaxError, UnexpectedToken}
...
import org.scalawag.bateman.json.parser.tokenizer.Tokenizer.TokenStream
...
object Eventizer {
  ...
  private def unexpected(expected: Seq[String])(in: TokenStream): EventStream =
    in.head match {
      case Right(t) => LazyList(UnexpectedToken(t, expected: _*).asLeft)
      case Left(e)  => LazyList(e.asLeft)
    }

  private def unexpected(expected: String)(in: TokenStream): EventStream =
    unexpected(Seq(expected))(in)

  private def unexpectedToken(expected: Token*)(in: TokenStream): EventStream =
    unexpected(expected.map(Token.describe))(in)
  ...
  private def fieldValue(next: State)(in: TokenStream): EventStream =
    in.head match {
      ...
      case _               => unexpectedToken(Colon(null))(in)
    }
  ...
  private def additionalFields(next: State)(in: TokenStream): EventStream =
    in.head match {
      ...
      case _                    => unexpectedToken(CloseBrace(null), Comma(null))(in)
    }
  ...
  private def additionalItems(next: State)(in: TokenStream): EventStream =
    in.head match {
      ...
      case _                      => unexpectedToken(CloseBracket(null), Comma(null))(in)
    }
  ...
  private def end(in: TokenStream): EventStream =
    in.head match {
      case Right(_: EndOfText) => LazyList.empty
      case _                   => unexpectedToken(EndOfText(null))(in)
    }
