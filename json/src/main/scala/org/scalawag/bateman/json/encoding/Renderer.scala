...
trait PrimitiveAppenders extends AppenderRenderer {
  ...
  private def appendStringCharacter(c: Char): AppenderState[Unit] =
    c match {
      ...
      case '\n'                         => append("\\n")
      case '\t'                         => append("\\t")
      case c if c < 0x001f              => append("\\u", pad(c.toHexString))
      case c if asciiOnly && c > 0x00ff => append("\\u", pad(c.toHexString))
...
object NoSpacesRenderer extends AppenderRenderer with PrimitiveAppenders {
  ...
  private def append(x: (String, JAny)): AppenderState[Unit] =
    for {
      _ <- append(JString(x._1))
      _ <- append(":")
      _ <- append(x._2)
    } yield ()
  ...
  protected override def append(x: JObject): AppenderState[Unit] =
    for {
      _ <- append("{")
      _ <- intersperse(comma)(x.fields.toList.map(append)).sequence
      _ <- append("}")
    } yield ()
  ...
  protected override def append(x: JArray): AppenderState[Unit] =
    for {
      _ <- append("[")
      _ <- intersperse(comma)(x.items.toList.map(append)).sequence
      _ <- append("]")
    } yield ()
}
...
trait SortedFields extends AppenderRenderer {
  protected abstract override def append(x: JObject): AppenderState[Unit] = {
    val s = JObject(x.fields.sortBy(_._1): _*)
    println(x.fields.map(_._1))
    println(s.fields.map(_._1))
    super.append(s)
  }
}
