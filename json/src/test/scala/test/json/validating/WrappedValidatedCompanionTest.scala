...
import org.scalawag.bateman.json.{InvalidValue, JNumber}
import test.json.validating.WrappedValidatedCompanionTest.NonNegativeInteger

object WrappedValidatedCompanionTest {
  final case class NonNegativeInteger private[NonNegativeInteger] (value: Int)

  object NonNegativeInteger extends WrappedValidatedCompanion[Int, NonNegativeInteger](new NonNegativeInteger(_)) {
...
class WrappedValidatedCompanionTest extends AnyFunSpec with Matchers {
  ...
  it("should accept NonNegativeInteger on decode") {
    pos.decode[NonNegativeInteger] shouldBe NonNegativeInteger.unsafe(1).rightNec
  }
  ...
  it("should reject NonNegativeInteger on decode") {
    neg.decode[NonNegativeInteger] shouldBe InvalidValue(neg, "has to be greater than zero").leftNec
  }
