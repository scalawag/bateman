...
object HListResourceEncoderFactory {
  ...
  /** Represents the state of encoding through the stack of individual field encoders. This class keeps track of both
    * the requested shape ([[includeSpec]], [[fieldsSpec]]) and the encoders that have been found
    * ([[attributesHandled]], [[relationshipsHandled]]) so that it can do error checking when it exhausts the
    * encoders ([[forHNil]]). If there are fields that have been requested which do not exist on the case class
    * (as evidenced by them not being "handled"), an error is raised. This is indicated by [[includeSpec]] or
    * [[fieldsSpec]] referencing a field that was not present in either [[attributesHandled]] or
    * [[relationshipsHandled]].
    *
    * @param in the case class field values to be encoded
    * @param resourceType the resource type of the resource being encoded
    * @param includeSpec the included relationships requested by the caller
    * @param fieldsSpec the fields requested by the caller
    * @param attributesHandled use to keep track of which attribute encoders have been found
    * @param relationshipsHandled use to keep track of which relationship encoders have been found
    * @tparam In
    */
        ...
        input.includeSpec.descend(jsonFieldName) match {
          ...
          case childIncludeSpec =>
            ...
            encodedTail
              .whenValid(
                head.traverse(
                  _.parTraverse(headEncoder.value.encodeResource(_, childIncludeSpec, input.fieldsSpec, JObject.Empty))
                )
              ) {
