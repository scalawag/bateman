    ...
    params => {
      ...
      (in, includeSpec, fieldsSpec, discriminators) =>
        in match {
          ...
          case Inl(h) =>
            // This is the type we've received as input, so encode it with our discriminator accumulated.
            ...
            val enrichedDiscriminators = params.addDiscriminator(discriminators, disc.value)
            effectiveHeadEncoder.encodeResource(h, includeSpec, fieldsSpec, enrichedDiscriminators)
        }
    }
}
