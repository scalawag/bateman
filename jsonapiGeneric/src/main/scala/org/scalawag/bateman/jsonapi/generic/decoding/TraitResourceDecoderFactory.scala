...
import org.scalawag.bateman.json.generic.decoding.{CoproductDecoderFactory, TraitDecoderFactory}
import org.scalawag.bateman.json.generic.{Config, DiscriminatorCollision, MemberLabels, TraitDeriverParams}
...
//trait TraitResourceDecoder[Out] extends JObjectDecoder[Out]
//
//object TraitResourceDecoder {
//  def apply[Out](implicit dec: TraitResourceDecoder[Out]): TraitResourceDecoder[Out] = dec
//}
...
object TraitResourceDecoderFactory {
  ...
  implicit def traitDecoder[Trait, Generic <: Coproduct](implicit
      ...
      decoderFactory: CoproductResourceDecoderFactory[Generic]
  ): TraitResourceDecoderFactory[Trait] = { params =>
    {
      ...
      (in, discriminatorFields) =>
        in(params.discriminatorLens).flatMap { disc =>
          val input = Input(in, disc, discriminatorFields + disc, Nil)
