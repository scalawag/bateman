...
import org.scalawag.bateman.json.generic.{DiscriminatorCollision, Discriminators, TraitDeriverParams, TraitUtils}
  ...
  inline given derived[T](using m: Mirror.SumOf[T]): TraitResourceDecoderFactory[T] =
    ...
    class TraitResourceDecoderFactoryImpl(
      ...
      def apply(params: TraitDeriverParams[JObjectDecoder]): JObjectDecoder[T] =
        ...
        // Pair each mapping with the decoder to invoke at runtime: the explicit decoder if one was
        // captured by the discriminator (layered routing), otherwise the leaf's own decoder.
        val discriminatorMappings: List[(JAny, JObjectDecoder[Any])] =
          decoders.zip(mappings).map { case (decoder, mapping) =>
            (mapping.value, mapping.explicit.getOrElse(decoder.asInstanceOf[JObjectDecoder[Any]]))
          }
