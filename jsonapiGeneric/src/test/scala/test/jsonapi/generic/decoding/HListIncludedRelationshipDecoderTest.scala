...
object HListIncludedRelationshipDecoderTest {
  ...
  object MyIdRelationshipDefaulted {
    case class MyClass(@IncludedRelationship a: MyRefObj = MyRefObj("A", "Ab"))
  }

  object MyNullableRelationshipDefaulted {
    case class MyClass(@IncludedRelationship a: Nullable[MyRefObj] = NotNull(MyRefObj("B", "Bb")))
  }

  object MyListRelationshipDefaulted {
    case class MyClass(@IncludedRelationship a: List[MyRefObj] = List(MyRefObj("C", "Cb"), MyRefObj("D", "Db")))
  }

  object MyOptionIdRelationshipDefaulted {
    case class MyClass(@IncludedRelationship a: Option[MyRefObj] = Some(MyRefObj("E", "Eb")))
  }

  object MyOptionNullableRelationshipDefaulted {
    case class MyClass(@IncludedRelationship a: Option[Nullable[MyRefObj]] = Some(NotNull(MyRefObj("F", "Fb"))))
  }

  object MyOptionListRelationshipDefaulted {
    case class MyClass(
        @IncludedRelationship a: Option[List[MyRefObj]] = Some(
          List(MyRefObj("G", "Gb"), MyRefObj("H", "Hb"), MyRefObj("I", "Ib"))
        )
    )
  }
}

class HListIncludedRelationshipDecoderTest extends HListDecoderTestBase {
  ...
  describe("MyIdRelationship") {
    ...
    relationshipDatumIsIncluded.succeedsWith(MyClass(MyRefObj("J", "Jb")))
  }
  ...
  describe("MyNullableRelationship") {
    ...
    relationshipDatumIsIncluded.succeedsWith(MyClass(NotNull(MyRefObj("J", "Jb"))))
  }
  ...
  describe("MyListRelationship") {
    ...
    relationshipDataAreIncluded.succeedsWith(MyClass(List(MyRefObj("K", "Kb"), MyRefObj("L", "Lb"))))
  ...
  describe("MyOptionIdRelationship") {
    ...
    relationshipDatumIsIncluded.succeedsWith(MyClass(Some(MyRefObj("J", "Jb"))))
  }
  ...
  describe("MyOptionNullableRelationship") {
    ...
    relationshipDatumIsIncluded.succeedsWith(MyClass(Some(NotNull(MyRefObj("J", "Jb")))))
  }
  ...
  describe("MyOptionListRelationship") {
    ...
    relationshipDataAreIncluded.succeedsWith(MyClass(Some(List(MyRefObj("K", "Kb"), MyRefObj("L", "Lb")))))
    relationshipDataHasSomeBadTypes.failsWithMultiple[MyClass] { in =>
      NonEmptyChain(
        JsonTypeMismatch(in(focus ~> 0).shouldSucceed, JObject),
        JsonTypeMismatch(in(focus ~> 2).shouldSucceed, JObject)
  ...
  describe("MyIdRelationshipDefaulted") {
    ...
    invalidJsonApiStructure[MyClass]()
    casesForCardinalityId[MyClass]()
    resourceIsAnEmptyObject.succeedsWith(MyClass(MyRefObj("A", "Ab")))
    relationshipsIsAnEmptyObject.succeedsWith(MyClass(MyRefObj("A", "Ab")))
    relationshipDatumIsIncluded.succeedsWith(MyClass(MyRefObj("J", "Jb")))
  }
  ...
  describe("MyNullableRelationshipDefaulted") {
    ...
    invalidJsonApiStructure[MyClass]()
    casesForCardinalityNullable[MyClass]()
    resourceIsAnEmptyObject.succeedsWith(MyClass(NotNull(MyRefObj("B", "Bb"))))
    relationshipsIsAnEmptyObject.succeedsWith(MyClass(NotNull(MyRefObj("B", "Bb"))))
    relationshipDataIsNull.succeedsWith(MyClass(Null))
    relationshipDatumIsIncluded.succeedsWith(MyClass(NotNull(MyRefObj("J", "Jb"))))
  }
  ...
  describe("MyListRelationshipDefaulted") {
    ...
    invalidJsonApiStructure[MyClass]()
    casesForCardinalityList[MyClass]()
    resourceIsAnEmptyObject.succeedsWith(MyClass(List(MyRefObj("C", "Cb"), MyRefObj("D", "Db"))))
    relationshipsIsAnEmptyObject.succeedsWith(
      MyClass(List(MyRefObj("C", "Cb"), MyRefObj("D", "Db")))
    )
    relationshipDataAreEmpty.succeedsWith(MyClass(Nil))
    relationshipDataAreMissingIds.failsWithMultiple[MyClass] { in =>
      NonEmptyChain(
    ...
    relationshipDataAreIncluded.succeedsWith(
      MyClass(List(MyRefObj("K", "Kb"), MyRefObj("L", "Lb")))
    )
    relationshipDataHasSomeBadTypes.failsWithMultiple[MyClass] { in =>
      NonEmptyChain(
        JsonTypeMismatch(in(focus ~> 0).shouldSucceed, JObject),
