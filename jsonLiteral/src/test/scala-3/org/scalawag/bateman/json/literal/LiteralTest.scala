...
import org.scalawag.bateman.json.{JAnyEncoder, JArray, JBoolean, JLocation, JNull, JNumber, JObject, JString}
...
class LiteralTest extends AnyFunSpec with Matchers {
  describe("JObject") {
    ...
    it("should allow any insertion that has an encoder") {
      ...
      val json: JObject = json"""{"foo": $insert}"""
  ...
  describe("JArray") {
    ...
    it("should allow any insertion that has an encoder") {
      ...
      val json: JArray = json"""[$insert]"""

      json shouldBe JArray(JString(insert.toString))
    }

    it("should include location info with no interpolations") {
      val json: JArray = json"""[ 4, true, null ]"""
  ...
  describe("other types") {
    it("should parse a JNumber") {
      val json: JNumber = json"8"
      json shouldBe JNumber("8", Some(JLocation(1, 1)))
    }

    it("should parse a JString") {
      val json: JString = json""""abc""""
      json shouldBe JString("abc", Some(JLocation(1, 1)))
    }

    it("should parse a JBoolean") {
      val json: JBoolean = json"true"
      json shouldBe JBoolean(true, Some(JLocation(1, 1)))
    }

    it("should parse a JNull") {
      val json: JNull = json"null"
      json shouldBe JNullImpl(Some(JLocation(1, 1)))
    }
  }
